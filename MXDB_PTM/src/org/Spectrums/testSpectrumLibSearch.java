...
public class testSpectrumLibSearch {
	...
	public static void testTopMatch(String spectrumFile, String spectrumFile2, String file, double tolerance){
		...
		spectrumFile2 = "..\\mixture_linked/linked_peptide_library/disulfide_lib/20111221_ananiav_DTT_IAA_lib1_90min_CID35.mzXML";
		//spectrumFile2 = "../mixture_linked/yeast_data/klc_010908p_yeast-digest.mzXML";
		tolerance = 0.05;
		file = "..\\mixture_linked\\database\\lib_disulfide.txt";
		...
		factory.setMaxCharge(5);
		factory.matchCharge = false;
		...
		PeakComparator pComp = RankBaseScoreLearner.loadComparator("../mixture_linked/yeast_NIST_lib_singlepep_win10_25.o");
		...
		for(;iterator.hasNext();){
			...
			//removeSUMOPeaks(s);
			System.out.println(s.spectrumName + "\t" + s.charge + "\t" + s.parentMass+"\tobserved spectrum has peaks: " + s.getPeak().size());
			s.computePeakRank();
			...
			System.out.println(s.spectrumName + "\t" + s.charge + "\tobserved spectrum has peaks: " + s.getPeak().size() 
					+ "\tnumb of cand:\t" + lib.getSpectrumList().size() 
					+ "\tpif:\t" + s.upperBound);
...
//			List<Spectrum> candidates = filteredLib.getSpectrumByPeaks(queryPeaks, 4);
	...
	public static CandidateSpectrumLibFactory getSUMOFactory(String peptideFile){
		...
		peptides2.add("Q-17.027QQTGG");
		peptides2.add("QQQTGG");
		//peptides2.add("Q+6.01QQTGG");
		//peptides2.add("Q-11.0170QQTGG");
		peptides2.add("QQTGG");
		peptides2.add("QTGG");
		peptides2.add("TGG");
		peptides2.add("GG");
	...
	public static CandidateSpectrumLibFactory getCrossLinkFactory(String peptideFile){
		...
		factory.insertPTM(42.010565, 1, 1);
		//factory.insertPTM(28, 1, 1);
		//factory.insertPTM(184.087, new char[]{'K'}, 1);
		//factory.insertPTM(1, new char[]{'Q'}, 1);
		//Mass.DSSLINKER_MASS = -116.0430;//136.0995;//138.0680;
		Mass.DSSLINKER_MASS = 166.1775;//138.0680;//
		CrossLinker CROSSLINKER = new CrossLinker(Mass.DSSLINKER_MASS, new int[]{CrossLinker.ANYPOSITION}, new int[]{CrossLinker.ANYPOSITION}, new char[]{'K'}, new char[]{'K'});
		...
		//CrossLinker CROSSLINKER = new CrossLinker(Mass.DSSLINKER_MASS, new int[]{CrossLinker.ANYPOSITION}, new int[]{CrossLinker.CTERM}, new char[]{'K'}, new char[]{'G'});
		System.out.println("start crosslinking");
	...
	public static void runBruteForaceCrossLinkSearch(String queryFile, String peptideFile, String scoreFile1, String scoreFile2, int minScan, int maxScan){
		...
		for(; it.hasNext();){
			...
			s.computePeakRank();
			List candidates = new ArrayList();
			candidates.addAll(factory.getSUMOCandidates(s, 0.5));
			//List<Peptide> candidates2 = factory.getCandidateByMass(s.parentMass-(Mass.C13-Mass.C12)/s.charge, s.charge, 0.05);
			System.out.println("Scan Number: " + s.scanNumber + "\t" + s.parentMass + "\t" +  s.charge + "\t" + " observed spectrum has peaks: " 
					+ s.getPeak().size() + " number of candidate peptides: " + candidates.size() + "\tpif:\t" + s.upperBound);
			SpectrumLibSearcher searcher = new SpectrumLibSearcher(candidates, scorer2);
			searcher.setSingleScorer(scorer1);
			searcher.topLinkedSpectra(s, 10);
		}
	...
	public static void main(String[] args){
		...
		//args[0] = "..\\mixture_linked\\linked_peptide_library/sumo_lib/human_sumo/Veronica_sumo_enrich/20120309/20120309_ananiav_HA_S1_2xchymo_37C_CID_top15.mzXML";
		args[0] = "..\\mixture_linked/msdata/human_sumo/Veronica_sumo_enrich/20120702_double_digest/20120702_SUMO_IP_T_IP_C.mzXML";
		//args[0] = "..\\mixture_linked\\msdata\\philAndrews\\Aldolase_BS3_032408/Aldolase_xlink_BS3_032508_scx10.mzXML";
		//args[0] = "..\\mixture_linked/msdata/philAndrews/PC1_model_peptides/Peptide_2_and_4_1.mzXML";
...
//		//args[1] = "A[FILVYW]K[ARDEHLKMFPSTYV][DE]T[ARDEHLKMFPSTYV]FRAK";
		//args[1] = "..\\mixture_linked\\database\\PC1_model_peptides.txt";
		args[1] = "..\\mixture_linked\\database\\mcl1_syn_peptides.txt";
		args[2] = "..\\mixture_linked\\yeast_linked_single_peptide_model.o";
		//args[3] = "..\\mixture_linked\\disulfide_linked_model.o";
