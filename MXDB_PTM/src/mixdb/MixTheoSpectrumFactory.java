...
public class MixTheoSpectrumFactory{
	...
	public static Map<String, Collection<IonType>> createMixPeptideMap(List<IonType> ionTypeList){
		...
		for(Iterator<IonType> it = ionTypeList.iterator(); it.hasNext();){
			...
			String key = pType.toString(); //Note to self: why use string as key and not the peptype directly??
	...
	/**
	 * takes two single-peptide theo-spectrum and transform into a mixture theospectrum
	 * by shifting all ion type index in low-abudnance peptide theo spectrum by a fix offset 
	 * @param th1
	 * @param th2
	 * @return
	 */
