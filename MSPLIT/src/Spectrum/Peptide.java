...
//	}
	...
	/**
	 * parsing string representation of peptides
	 * @param pep
	 * @return
	 */
	...
	/**
	 * parse string representation of peptides handling different format of PTM specification
	 * @param pep
	 * @return
	 */
			...
			if(pep.charAt(i) == '+' || pep.charAt(i) == '-' || pep.charAt(i) == '['){
				...
				if(ptm.length() > 0){					                                                                                
					if(isSpectraSTFormat){
						ptmmass = Double.parseDouble(ptm.toString());
						if(ptmPos > 0){
							ptmmass = ptmmass - (int)Mass.getAAMass(raw.charAt(ptmPos-1));
						}
						isSpectraSTFormat = false;                                                            //it has residue+ptm rather than just the ptm offset
					}else{
						ptmmass  = Double.parseDouble(ptm.toString());
					}
					if(Math.abs(ptmmass) > 0.1){
						masses.add(ptmmass);
				...
				if(ptmPos == 0){   //not differetiating n-term of mod on first residue
					ptmPos = 0;    //makes all mod on n-term as if they are on first residue
				}
