...
/**
 * A Map implementation back-by a random access file, so
 * the Map can be large but only a relative small portion of it are kept
 * in the memory.  This can be adjusted the keepObjects in the class
 * @author Jian Wang
 *
 */
public class LargeHashMap {
	...
	public void finalizeObjectFile(){
		try{
			ByteArrayOutputStream out1;
			ObjectOutputStream out2;
			long current = RAF.getFilePointer();
