...
public class MXGF {
	...
	public MXGF(double maxMass, double maxMass2, double maxScore, Map<Double, Double> aaFreq){
		this.maxMass = Math.ceil(maxMass);
		this.maxMass2 = Math.ceil(maxMass2);
	...
	private void initialize(){
		...
		if(this.minScore == 0)
			minScore = MINSCORE;
		this.scoreHi = new double[((int)Math.ceil(maxMass))+2];
		this.scoreLow = new double[((int)Math.ceil(maxMass2))+2];
		this.getAAStat();
	...
	private void getAAStat(){
		...
		for(int i = 0; i < Mass.aaMap.length; i++){
			if(Mass.aaMap[i] > 0 && Mass.aaMap[i] != 111.0316){
				this.validAAMass[j] = (int)Math.round(Mass.aaMap[i]*0.9995);
	...
	public void computeCondPair(int[] masses){
		...
		table[0][1][offSet] = 1;
		int minS = (int)Math.floor(minScore);
		double[] oldScores = new double[masses.length];
		for(int i = 0; i < masses.length; i++){
			if(masses[i] <= this.maxMass2){
				oldScores[i] = this.scoreLow[masses[i]];
		...
		for(int i = 0; i < masses.length; i++){
			if(masses[i] <= this.maxMass2){
				this.scoreLow[masses[i]] = oldScores[i];
			}
		}
	}
	...
	public double getJointProb(double score, double mass1, double mass2){
		...
		int m1 = (int)Math.round(this.scaleMass(mass1));
		int m2 = (int)Math.round(this.scaleMass(mass2));
...
//		double totalScore = sComp.compare(t, prmSpect);
		...
		double pm2 = p2.getParentmass() * p2.getCharge();
		double pm = pm1 > pm2 ? pm1 : pm2;
		MXGF mixgf = new MXGF(pm1, pm2, 200, null);
		mixgf.setScoreHi(scores);
		mixgf.setScoreLow(scores2);
//		mixgf.computePair();
//		double jProb = mixgf.getJointProb(totalScore[0], 
...
//				(p2.getParentmass()*p2.getCharge()-Mass.PROTON_MASS*p2.getCharge()-Mass.WATER));
		...
		double[] prob = mixgf.getSpecProb(totalScore[1], totalScore[2],
				...
				(p2.getParentmass()*p2.getCharge()-Mass.PROTON_MASS*p2.getCharge()-Mass.WATER));
		if(s.scanNumber > 0){
			System.out.println("Scan " + s.scanNumber + "\t" + peptide + "\t" + peptide2 + "\t" + score +"\t" + score2 + "\t" + totalScore[0] + "\t" + totalScore[1] + "\t" + totalScore[2] + "\t" + prob[0] + "\t" + prob[1] + "\t" + jProb);
		}else{
			String id = s.spectrumName.replace(" & ", "&");
			System.out.println(id + "\t" + s.spectrumName + "\t" + peptide + "\t" + peptide2 + "\t" + score +"\t" + score2 + "\t" + totalScore[0] + "\t" + totalScore[1] + "\t" + totalScore[2] + "\t" + prob[0] + "\t" + prob[1] + "\t" + jProb);
		}
		return new double[]{score, score2, totalScore[0], totalScore[1], totalScore[2], prob[0], prob[1], jProb};
	}
	...
	public static void testComputeMSGF2(String spectrumFile, String annotationFile, String scorerFile){
		List<String> resultLines = Utils.FileIOUtils.createListFromFile(annotationFile);
		Map<Integer, String[]> table = new HashMap<Integer, String[]>();
		int scanColumn = 2;
		int peptideColumn = 10;
		for(int i = 0;  i < resultLines.size(); i++){
			String result = resultLines.get(i);
			if(result.startsWith("#")){
				continue;
			}
			String[] tokens = result.split("\\t");
			table.put(Integer.parseInt(tokens[scanColumn]), tokens);
		}
		//LargeSpectrumLibIterator<Spectrum> it = new LargeSpectrumLibIterator(spectrumFile);
		MZXMLReader it = new MZXMLReader(spectrumFile);
		PeakComparator comp = RankBaseScoreLearner.loadComparator(scorerFile);
		SpectrumComparator scorer = new SimpleProbabilisticScorer(comp);
		int counter = 0;
		while(it.hasNext()){
			...
			if(table.containsKey(s.scanNumber)){
				String[] result = table.get(s.scanNumber);
				String peptide = result[peptideColumn];
				//peptide = peptide.split("\\.")[1];
				//peptide = peptide+"."+result[5];
	...
	public static void testSimulatedMXGF(String annotationFile, String spectrumFile, String scorerFile, double alpha, int minCount, int maxCount){
		...
		long start = (new GregorianCalendar()).getTimeInMillis();
		for(int i = 0; i < resultLines.size(); i++){
			String[] tokens = resultLines.get(i).split("\\s+");
...
//			}
			...
			//computeMXGF(s1, peptide1, peptide2, scorer);
			computeMXGF(s1, decoy1, decoy2, scorer);
		}
		System.out.println("matching " + 500 + " spectra in time: " + (new GregorianCalendar().getTimeInMillis()- start)/1000 + "secs");
	}
	...
	public static void testComputeMXGF(String resultFile, String spectrumFile, String scorerFile, int minScan, int maxScan){
		...
		for(Iterator<String> it = resultLines.iterator(); it.hasNext();){
			...
			double[] scores = computeMXGF(s, tokens[7], tokens[9], scorer);
			System.out.println(line + "\t" + scores[0] +"\t" +  scores[1] + "\t" + scores[2] + "\t" + scores[3] + "\t" + scores[4] + "\t" + scores[5] + "\t" + scores[6] + "\t" + scores[7]);
			double[] scores2 = computeMXGF(s, tokens[9], tokens[7], scorer);
			System.out.println(line + "\t" + scores2[0] +"\t" +  scores2[1] + "\t" + scores2[2] + "\t" + scores2[3] + "\t" + scores2[4] + "\t" + scores2[5] + "\t" + scores2[6] + "\t" + scores2[7]);
		}
		System.out.println("matching " + 500 + " spectra in time: " + (new GregorianCalendar().getTimeInMillis()- start)/1000 + "secs");
	}
	...
	public static void main(String[] args){
		...
		//testComputeMSGF();
//		args[0] = "../mixture_linked/yeast5_MSPLITDB_svmclass.txt";
//		args[1] = "../mixture_linked/yeast_data/klc_122007p_yeast_digest5_071223083100.mzXML";
//		args[2] = "../mixture_linked/yeast_simmix_alpha_generic_12_25.o";
//		args[2] = "../mixture_linked/test.o";
//		args[3] = "1";
//		args[4] = "18000";	
...
////		args[2] = "..\\mixture_linked\\MSGFSearches\\testprmout.txt";
		...
		//testComputeMSGF2(args[0], args[2], args[1]);
