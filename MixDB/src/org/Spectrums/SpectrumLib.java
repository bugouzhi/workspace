	...
	public SpectrumLib createRandomMix(int size, double scale, double scale2, double minSim, double maxSim, double massDiff, boolean toVector){
		...
		while(counts < size){
				...
				if((s1.cosineSim1(s2) >= minSim) && (s1.cosineSim1(s2) < maxSim)  //also require same charge for now
						...
						&& s1.charge <= 3
						&& s2.charge <= 3
						&& !s1.peptide.equals(s2.peptide) //){  //we do not mix two same spectrum
						&& s1.peptide.length() < 27
						&& s2.peptide.length() < 27){
...
//		);
	...
	public static void main(String[] args){
		if(args.length != 3){
			...
			//args[1]="../mixture_linked/yeast_data/klc_010908p_yeast-digest.mgf";
			//args[2] = "0.3";
