...
public class CopyOfSpectrum implements Comparable<CopyOfSpectrum>, Serializable{
	...
	public CopyOfSpectrum(){ //create dummy spectrum
	...
	public CopyOfSpectrum(Vector peaks, String peptide, double pm, int charge,
	...
	public CopyOfSpectrum(Vector peaks, String peptide, String name, double pm, int charge,
	...
	public CopyOfSpectrum(String spectrumFile, String format){
		if(format.equals("MGF")){
			readSpectrumFromMGF(spectrumFile);
		}
	}
	
	public CopyOfSpectrum readSpectrumFromNIST(String fileName){
		return new CopyOfSpectrum();
	}
	
	//create a new spectrum by combining two spectrum together
	//note this is a simulated spectrum, so some of the field might
	//not make sense 
	public CopyOfSpectrum(CopyOfSpectrum s1, CopyOfSpectrum s2, double scale1, double scale2){
	...
	public CopyOfSpectrum(CopyOfSpectrum s1, CopyOfSpectrum s2, double scale1, double scale2, boolean toVector){
		if(toVector){
			s1 = s1.toVector(BINWIDTH, MINMASS, MAXMASS);
			s2 = s2.toVector(BINWIDTH, MINMASS, MAXMASS);
			s1.computePeakRank();
	...
	public CopyOfSpectrum(CopyOfSpectrum s1, CopyOfSpectrum s2, double scale1, double scale2, boolean toVector, boolean merge){
	...
	private void mergePeaks(CopyOfSpectrum mix, double tolerance){
	...
	public CopyOfSpectrum(CopyOfSpectrum s1, CopyOfSpectrum s2){
		this(s1,s2,1,1);
	}
	...
	public int compareTo(CopyOfSpectrum s){
	...
	public CopyOfSpectrum toNormVector(double binWidth, double minMass, double maxMass){
		CopyOfSpectrum newSpectrum = toVector(binWidth, minMass, maxMass);
		newSpectrum.normalize();
		return newSpectrum;
	}
	
	public CopyOfSpectrum toNormVector(){
		
		return this.toNormVector(BINWIDTH, MINMASS, MAXMASS);
	}
	
	public CopyOfSpectrum toVector(double binWidth, double minMass, double maxMass){
		...
		return new CopyOfSpectrum(newPeaks, this.peptide, this.parentMass, this.charge,
	...
	public double cosineSim(CopyOfSpectrum s1){
		double product = 0;
	...
	public double cosineSim1(CopyOfSpectrum s1){
		double product = 0;
	...
	public double shiftCosineSim(CopyOfSpectrum s1){
		CopyOfSpectrum original = this.toNormVector();
		this.shiftSpectrum(0.5);
		CopyOfSpectrum rightShift = this.toNormVector();
		this.shiftSpectrum(-1.0); //left shift original spectrum by -0.5 (0 + 0.5 - 1.0) = -0.5
		CopyOfSpectrum leftShift = this.toNormVector(); 
		this.shiftSpectrum(0.5); //restore the original spectrum
		CopyOfSpectrum vS1 = s1.toNormVector();
	...
	public double shareSim(CopyOfSpectrum s1){
	...
	public double shareSim(CopyOfSpectrum s1, int mode){
		double product = 0;
	...
	public double projectedCosine(CopyOfSpectrum s1){
		double product = 0;
		double magnitude = this.magnitude();
	...
	//but here we consider only those bins
	//that are non-zero in this vector
	public double projectedCosine1(CopyOfSpectrum s1){
		double product = 0;
		double magnitude = this.sumOfPeaks();
	...
	public double alpha(CopyOfSpectrum a, CopyOfSpectrum b) {		
		double A, B, C, D, E ;		
		C = this.dot(a) ;
		D = a.dot(b) ;
		E = this.dot(b) ;
...
//		}
	...
	public double maxScore(CopyOfSpectrum a, CopyOfSpectrum b, double alpha){
		alpha = Math.pow(alpha, 0.5);  //needed this to take into account of sqrt transformation
		CopyOfSpectrum answerMix1 = new CopyOfSpectrum(a, b, alpha, 1) ;
		CopyOfSpectrum answerMix2 = new CopyOfSpectrum(b, a, alpha, 1) ;
		//answerMix1.windowFilterPeaks(10, 25);
		//answerMix2.windowFilterPeaks(10, 25);
		CopyOfSpectrum baseAnswer = new CopyOfSpectrum(a, b, 1, 0.1); //in case it is a single-match
	...
	public double shiftMaxScore(CopyOfSpectrum a, CopyOfSpectrum b, double alpha){
	    CopyOfSpectrum origA = a.toNormVector();
	    a.shiftSpectrum(0.5);
	    CopyOfSpectrum rightA = a.toNormVector();
	    a.shiftSpectrum(-1.0);
	    CopyOfSpectrum leftA = a.toNormVector();
	    a.shiftSpectrum(0.5);
	    CopyOfSpectrum origB = b.toNormVector();
	    b.shiftSpectrum(0.5);
	    CopyOfSpectrum rightB = b.toNormVector();
	    b.shiftSpectrum(-1.0);
	    CopyOfSpectrum leftB = b.toNormVector();
	    b.shiftSpectrum(0.5);
	    CopyOfSpectrum mix = this.toNormVector();
	...
	public double inverseMaxScore(CopyOfSpectrum a, CopyOfSpectrum b, double alpha){
		//alpha = 10;
		//b.filterPeaks(20);
		CopyOfSpectrum answerMix = new CopyOfSpectrum(a, b);
		CopyOfSpectrum mix = this.toNormVector();
	...
	public void upscale(CopyOfSpectrum s1, CopyOfSpectrum s2, double alpha){
	...
	public double dot(CopyOfSpectrum s1) {
		double product = 0;
		double mz1, mz2; 
		int i = 0, j = 0;
	...
	public void removeSharePeaks(CopyOfSpectrum s){
	...
	public double residual(CopyOfSpectrum s){
		Iterator<Peak> p1 = this.peaks.iterator();
		Iterator<Peak> p2 = s.getPeak().iterator();
		Peak peak1 = null, peak2 = null;
		int exp = 4;
...
//		System.out.println("s has intensity: " + s.magnitude());
	...
	public void subtractSharePeaks(CopyOfSpectrum s){
		Iterator<Peak> p1 = this.peaks.iterator();
		Iterator<Peak> p2 = s.getPeak().iterator();
		Peak peak1 = null, peak2 = null;
		if(p1.hasNext() && p2.hasNext()){
	...
	private int addNeighborPeaks(double deltaM, List<Peak> v, Peak p, int beginInd){
	...
	private void removeNonNeighborPeaks(double deltaM, List<Peak> v, Peak p){
	...
	public static void testReadMGF(){
		...
		CopyOfSpectrum msms = new CopyOfSpectrum();
	...
	public static void testtoVector(){
		...
		CopyOfSpectrum msms = new CopyOfSpectrum();
		msms.readSpectrumFromMGF(filename);
		CopyOfSpectrum s = msms.toVector(50, 100, 500);
	...
	public static void testVectorRep(){
		...
		CopyOfSpectrum msms = new CopyOfSpectrum();
		msms.readSpectrumFromMGF(filename);
		CopyOfSpectrum msms2 = new CopyOfSpectrum();
	...
	public static void testMixSpect(){
		...
		String filename = "testspectrum.mgf";
		CopyOfSpectrum s1 = new CopyOfSpectrum();
		s1.readSpectrumFromMGF("testspectrum.mgf");
		CopyOfSpectrum s2 = new CopyOfSpectrum();
		s2.readSpectrumFromMGF("testspectrum2.mgf");
		CopyOfSpectrum msms12 = new CopyOfSpectrum(s1, s2);
		s1=s1.toNormVector();
		s2=s2.toNormVector();
		System.out.println(s1);
		System.out.println(s2);
	...
	public static void testShiftCosine(){
		...
		CopyOfSpectrum s = lib1.getSpectra("spec_3182.dta..1").get(0);
		CopyOfSpectrum m = mixlib.getSpectra("spec_186.dta..1").get(0);
		CopyOfSpectrum sCopy = s;
