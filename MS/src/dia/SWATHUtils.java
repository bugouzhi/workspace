...
public class SWATHUtils {
	...
	/**
	 * Check whether precursor is within the swath window
	 * note precursor mass for swath spectrum is set a strange place, for a typical 25Da window
	 * we check P-4 to P + 21 where P is the precursor value
	 * @param ms2
	 * @param precursor
	 * @param windowWidth
	 * @return
	...
	protected static boolean checkMass(double ms2, double precursor, double tolerance, int mode){
		return SpectrumUtil.checkMass(ms2, precursor, tolerance, mode);
	}
	
	
	/**
	 * Create a histogram of peak intesnity
	 * @param s
	 * @param min
	 * @param max
	 * @param binWidth
	...
	/**
	 * get the corresponding swath ms1 scan for a given swath ms2 scan
	 * @param s
	 * @return
	 */
	...
	/**
	 * Output targetd assay library for peakview, based on a list of identified swath spectra
	 * For each peptide, fragment ion peaks used to generated assays are from the library, 
	 * but RT is from the actual swath run where the peptide is identified
	 * @param specList
	 * @param outFile
	 */
...
//						outbuffer.append("True" + "\t");
	...
	/**
	 * Similar to above but export assay library in openswath format
	 * @param specList
	 * @param outFile
	 */
...
//							System.out.println("Index: " + index);
	...
	/**
	 * Export assay library in skyline format
	 * @param specList
	 * @param outFile
	 */
...
//							System.out.println("Index: " + index);
	 ...
	 * search tools such as MSPLIT-DIA, the identified peptides are then used to do targeted extraction from swath
	 * @param libFile
	 * @param SpectrumFile
	 * @param resultsFile
	 */
	...
	public static void getIonLibrary(String libFile, String spectrumFile, String resultsFile, String fastaFile, String outFile, String outFormat){
		...
		Map<String, List<String>> pMap = protID.getPeptideMap();
	...
	/**
	 * Given an input spectrum file, perform window filtering (top 15 per +/-25 Da window)
	 * and output the filtered spectra
	 * @param swathFile
	 * @param outFile
	 */
