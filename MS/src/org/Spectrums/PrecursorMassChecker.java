...
public class PrecursorMassChecker {
	...
	public PrecursorMassChecker(String filename, String annotationFile){
		...
		this.reader = new MZXMLReader(this.filename);
	...
	private String[] parseResultLine(String line){
		...
		return new String[]{tokens[1], tokens[4]+"."+tokens[6]};
	}
	
	private String[] parseMixtureResultLine(String line){
		String[] tokens = line.split("\\s+");
		return new String[]{tokens[3], tokens[7], tokens[9]};
	}
...
//			}
	...
	private double getPIF(Peptide p, int scanNum){
		double remain=0, pif=0;
		if(p!=null){
			remain = (p.getParentmass() - 400) % 25;
			int scanMS1 = getPrecursorScan(scanNum);
			pif = this.reader.getPIF(scanMS1, p.getParentmass(), 30, -1*(remain+0.5), (25-remain+0.5), 4, p.getCharge())[2]; //the 0.5 Da on either side of window is for handling overlap btw two consecutive swath window
		}
		return pif;
	}
	...
	private double[] computeMinDiff(Peptide p, int scanNum, int mode, boolean detail){
		...
		double[] abundance = new double[profile.length];
		...
		for(int i = 0; i < peaks[0].length; i++){
			for(int j = 0; j < profile.length; j++){
				...
				abundance[j] = Math.abs(diff) < Math.abs(minDiff[j]) ? peaks[1][i] : closest[j];
				minDiff[j] = Math.abs(diff) < Math.abs(minDiff[j]) ? diff(peaks[0][i],  profile[j], 2) : minDiff[j];
			}
		}
		//getting relative intensity
		double pif = 0;
		double remain = 0;
		...
		if(detail){
			...
			System.out.print(" total matched: " + count + "\tpif:\t" + pif);
		}
...
//				+	"closest match ");
	...
	private double[] computeMinDiff(Peptide p, Scan MS1, int mode, boolean detail, double center, double width){
		...
		removeNoisePeaks(peakList, 0.005);
...
//	}
	...
	//we also obtain one isotoic peaks to the left to make sure
	//another precursor starting earlier is NOT presented
	...
	public static void testCheckPrecursorProfile(){
		...
		String fileName = "..\\mixture_linked\\msdata/gringar/swath_development/14348_UPS2_4pm_Ecolilysate_SWATH_5600.mzXML";
