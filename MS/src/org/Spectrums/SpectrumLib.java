...
public class SpectrumLib implements Iterable, Serializable{
	...
	public SpectrumLib readSpectrumsFromMGF(String fileName){
		try{
			...
			int counter=0;
			while(success){
				if(spectrumLibrary.containsKey(getPeptideKey(s))){
					v = (Vector)spectrumLibrary.get(getPeptideKey(s));
				}else{
					v = new Vector();
				}
				...
				counter++;
				if(counter % 10000 == 0){
					System.out.println("Parsed spectrum: " + counter);
				}
			}		 
...
//	}
	...
	public void filterPeaks(double intFract){
		...
		while(it.hasNext()){
			Spectrum s = it.next();
			int topN = s.explainedIntensity2(intFract);
	...
	//filtering peaks that is not within this mass range
	public void filterPeakByMass(double minMass, double maxMass){
		Iterator<Spectrum> it = this.iterator();
		while(it.hasNext()){
			Spectrum s = it.next();
			s.filterPeaksByMass(minMass, maxMass);
		}
	}
	
	//remove peaks that fall in this mass range
		public void removePeakByMass(double minMass, double maxMass){
			Iterator<Spectrum> it = this.iterator();
			while(it.hasNext()){
				Spectrum s = it.next();
				s.removePeaksInMass(minMass, maxMass);
			}
		}
	...
	public void windowFilterPeaks(int top, double winSize){
		Iterator<Spectrum> it = this.iterator();
		while(it.hasNext()){
			it.next().windowFilterPeaks2(top, winSize);
		}
	}
	
	public void mergeSpectrum(double tolerance){
		Iterator<Spectrum> it = this.iterator();
		while(it.hasNext()){
			Spectrum curr = it.next();
			curr.mergePeaks(curr, tolerance);
		}
	}
	...
	public SpectrumLib Divide(){
		...
		while(it.hasNext()){
			...
			if(v.size() > 1){
				...
				vNew.add(v.get(0));
				newTable.put(getPeptideKey(v.get(0)), vNew);
				v.remove(0);
			}
		}
		this.spectrumList = this.getAllSpectrums();
...
//						&& s2.charge <= 3){  //we do not mix two same spectrum
					...
					vNew = (new Vector());
					vNew.add(mixture);
					if(!newTable.containsKey(mixture.peptide)){
						newTable.put(mixture.peptide, vNew);
						counts++;
					}
				}		
	...
	public void createMix(String outfile, int size, double minScale, double minSim, double maxSim, double massDiff, boolean toVector, int maxRepeat){
		try{
			BufferedWriter bo = new BufferedWriter(new FileWriter(outfile));
			//Hashtable <String, List <Spectrum>> newTable = new Hashtable();
			//List <List <Spectrum>> v = new Vector(this.spectrumLibrary.values());
			List <Spectrum> specList = this.getSpectrumList(); 
			Spectrum mixture, s1, s2;
			int counts = 0, repeat = 0;
			for(int i = 0; i < specList.size(); i++){
				...
				for(int j = i+1; j < specList.size(); j++){
					...
					if((s1.cosineSim(s2) >= minSim) && (s1.cosineSim(s2) < maxSim)  
						...
						double scale = Math.random();
						scale = scale*(1.0-minScale)+minScale;
					   	mixture = new Spectrum(s1, s2, 1.0, scale, toVector);
					   	mixture.score = s1.cosineSim(s2);
						counts++;
						repeat++;
						...
						bo.write("\n");
						if(counts % 1000 == 0){
							System.out.println("finished: " + counts);
						}
					}
					if(repeat >= maxRepeat){
						break;
...
//		);
	...
	public static void runSearch(SpectrumLib lib, String mixtureLibFile, double parentMassTolerance, String resultFile){
		...
		Iterator<Spectrum> it = new MZXMLReader(mixtureLibFile);
	...
	public static void main(String[] args){
		if(args.length != 3){
			...
			if(filename.contains(".sptxt")){
				lib1 = new SpectrumLib(filename, "splib");
			}else if(filename.contains(".msp")){
				lib1 = new SpectrumLib(filename, "MSP");
			}else if(filename.contains(".mgf")){
				lib1 = new SpectrumLib(filename, "MGF");
			}else{
				System.err.println("Unrecognized spectral library format");
				return;
			}
			...
			//mixlib.scaleSpectrumMass(0.9995);
			runSearch(lib1, fileMix, parentmass, "abc");
		}
	}	
